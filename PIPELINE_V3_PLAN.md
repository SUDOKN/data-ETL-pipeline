...
- **3.1** Mention node: flat payloads, floor-scan hold (tier 1), discovery stats
  (tier 2).
- **3.2** Synthesis node: soft-cutoff batching, groups whole, ~~dispositions parse~~ one synthesis per record with a FOCAL FORM (D15 as amended 2026-08-22), `location` as an A/B arm, snippet-radius knob on the collector.
