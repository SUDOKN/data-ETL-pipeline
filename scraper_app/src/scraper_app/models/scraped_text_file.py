...
class ScrapedTextFile(BaseModel):
    ...
    urls_failed: int
    success_rate: float
    is_valid: bool  # Default to True; can be overridden in factory method

    @field_serializer("text", mode="plain")
    def _serialize_text(self, v: str) -> str:
        # Only first 100 words in dumps; full text remains accessible via instance.text
    ...
    async def delete_permanently_if_possible(self, s3_client) -> None:
        ...
        await delete_scraped_text_from_s3_by_etld1(
            s3_client, self.etld1, self.s3_version_id
        )

    @classmethod
    async def upload_to_s3_and_create(
        cls, s3_client, batch: Batch, scrape_result: ScrapingResult, mfg_etld1: str
    ) -> ScrapedTextFile:
        is_valid = ScrapingResult.is_scrape_valid(
