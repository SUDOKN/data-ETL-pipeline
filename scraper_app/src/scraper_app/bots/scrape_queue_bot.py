        ...
        while True:
            ...
            try:
                ...
                if manufacturer:
                    ...
                    if manufacturer.scraped_text_file_version_id:
                        ...
                        if existing_txt_file:
                            ...
                            await delete_item_from_scrape_queue(
                                sqs_client, receipt_handle
                            )
                            continue
                        else:
                            print(
                ...
                task = asyncio.create_task(
                    scrape_manufacturer(
                        current_timestamp, s3_client, scraper, item, manufacturer
                    )
                )
                concurrent_manufacturers.add(task)

                def make_on_task_done(item: ToScrapeItem, receipt_handle, sqs_client):
                    def on_task_done(task):
                        concurrent_manufacturers.discard(task)
                        asyncio.create_task(
                            delete_item_from_scrape_queue(sqs_client, receipt_handle)
                        )
                        asyncio.create_task(
                ...
                task.add_done_callback(
                    make_on_task_done(item, receipt_handle, sqs_client)
                )

            except Exception as e:
                print(
                    f"Error before scraping manufacturer {item.manufacturer_url}: {e}"
                )
            ...
            print(f"Scraping URL: {item.manufacturer_url}")
            current_timestamp = get_current_time()
...
async def scrape_manufacturer(
    ...
    try:
        scraped_text = await scraper.scrape(
            item.manufacturer_url, item.manufacturer_url
        )
        print(
