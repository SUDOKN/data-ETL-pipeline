...
class AddressReconcileNode(ReconcileNode[BasicFieldTypeEnum.addresses]):
    ...
    async def wipe_down(
        ...
        logger.info(
            f"Reconciled address {self.field_type.name} data for manufacturer {deferred_mfg.etld1}. "
        ...
        logger.info(
            f"Cleaned up GPTBatchRequests for address of {self.field_type.name} for manufacturer {deferred_mfg.etld1}. "
            f"Attempting to clear deferred_mfg address extraction field."
        )
