...
async def get_all_api_key_bundles() -> list[APIKeyBundle]:
    ...
    """
    return await APIKeyBundle.find(APIKeyBundle.exhausted == False).to_list()
...
async def find_inactive_api_keys() -> list[APIKeyBundle]:
    ...
    Find all available API keys.
    """
    return await APIKeyBundle.find(
        APIKeyBundle.latest_external_batch_id == None
    ).to_list()
...
async def _find_all_api_keys_by_names(names: list[str]) -> dict[str, APIKeyBundle]:
    ...
    keys = await APIKeyBundle.find(In(APIKeyBundle.label, names)).to_list()
