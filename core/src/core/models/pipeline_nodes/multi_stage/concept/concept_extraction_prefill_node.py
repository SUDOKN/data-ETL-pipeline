...
from core.utils.dict_diff import find_diffs
    ...
    async def execute(
        ...
        if not bool(getattr(deferred_mfg, self.field_type.name)):
            ...
            if deferred_concept_extraction.metadata.chunk_strat != self.chunk_strategy:
                ...
                raise ValueError(
                    f"Cannot proceed {__class__.__name__} for mfg:{mfg.subject_unique_id} as "
                    f"Metadata mismatch for {mfg.subject_unique_id}.{self.field_type.name} — differing fields: {diffs}"
                )

            logger.info(
                f"Chunking already done, resuming extraction for mfg_etdl1:{mfg.subject_unique_id}"
            )
