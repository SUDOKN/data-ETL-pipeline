...
class BaseLLMExtractionNode(BaseNode[LLMExtractedFieldTypeVar, ResultT]):
    ...
    async def execute(
        ...
        logger.debug(
            f"[{mfg.subject_unique_id}] 🔄 Executing {self.__class__.__name__} for field '{self.field_type.name}'"
        )
        ...
        await self.embed_request_ids(
            subject_unique_id=mfg.subject_unique_id,
        ...
        if missing_req_ids:
            logger.info(
                f"[{mfg.subject_unique_id}] 🆕 {self.__class__.__name__}: Missing requests detected for '{self.field_type.name}'. Creating batch requests..."
            )
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] ✅ Created {len(batch_requests)} batch requests for {self.__class__.__name__} ('{self.field_type.name}')"
            ...
            await bulk_upsert_gpt_batch_requests_with_only_req_bodies(
                batch_requests=batch_requests, mfg_etld1=mfg.subject_unique_id
            )
        else:
            logger.debug(
                f"[{mfg.subject_unique_id}] ✓ {self.__class__.__name__}: All requests already exist for '{self.field_type.name}'"
            )
        ...
        if eager:
            ...
            incomplete_requests = (
                await find_incomplete_gpt_batch_requests_by_custom_ids(
                    deferred_mfg.subject_unique_id, list(all_request_ids)
                )
            )
            logger.info(
                f"[{mfg.subject_unique_id}] 🚀 Eager execution enabled. Dispatching {len(incomplete_requests)} batch requests for {self.__class__.__name__} ('{self.field_type.name}') immediately."
            )
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] ✅ Eagerly dispatched {len(incomplete_requests)} batch requests for {self.__class__.__name__} ('{self.field_type.name}') with {modified_count} successful response recordings and {failed_updates} failed updates."
            )
        ...
        if await self.are_all_requests_complete(
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] ✅ {self.__class__.__name__} is COMPLETE for '{self.field_type.name}'. "
            ...
            if self.next_node:
                ...
                if isinstance(self.next_node, BaseLLMExtractionNode):
                    logger.info(
                        f"[{mfg.subject_unique_id}] ➡️  Proceeding to next ExtractionNode: {self.next_node.__class__.__name__}"
                    )
                elif isinstance(self.next_node, ReconcileNode):
                    logger.info(
                        f"[{mfg.subject_unique_id}] ➡️  Proceeding to ReconcileNode: {self.next_node.__class__.__name__}"
                    )
            ...
            logger.debug(
                f"[{mfg.subject_unique_id}] ⏸️  {self.__class__.__name__} is NOT complete for '{self.field_type.name}'. "
