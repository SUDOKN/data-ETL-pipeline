...
class BaseLLMRecursiveExtractionNode(
    ...
    async def execute(
        ...
        while True:
            ...
            logger.info(
                f"Embedding request ids for {self.__class__.__name__} ('{self.field_type.name}') for {mfg.subject_unique_id}"
            )
            ...
            logger.info(
                f"Saving deferred_mfg after embedding request ids for {mfg.subject_unique_id}"
            )
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] After embedding all request ids for {self.__class__.__name__} ('{self.field_type.name}'), missing_req_ids:{[missing_req_ids]}"
            )
            ...
            await bulk_upsert_gpt_batch_requests_with_only_req_bodies(
                ...
                mfg_etld1=mfg.subject_unique_id,
            )

            logger.info(
                f"[{mfg.subject_unique_id}] 🚀 Eager execution enabled. Dispatching {len(batch_requests)} batch requests for {self.__class__.__name__} ('{self.field_type.name}') immediately."
            )
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] ✅ Eagerly dispatched {len(batch_requests)} batch requests for {self.__class__.__name__} ('{self.field_type.name}') with {modified_count} successful response recordings and {failed_updates} failed updates."
            )
        ...
        if await self.are_all_requests_complete(
            ...
            logger.info(
                f"[{mfg.subject_unique_id}] ✅ {self.__class__.__name__} converged for '{self.field_type.name}'. "
