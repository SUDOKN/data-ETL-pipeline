"""Phase 3.1 of pipeline v3 (amended 2026-08-22; location coverage retired
with the location-stage merge, 2026-09-03): the fold as a dump reader meets
it — code-collected mentions, the per-window collection — and the request
witness for stored pre-merge per-sub-window mention requests."""

from types import SimpleNamespace
from typing import cast

from core.utils.aggregation_fold import WindowInput, collect_window, fold_document
...
def test_fold_dump_carries_groups_forms_mentions_and_the_window_report():
    ...
    intro = items[0]
    result = fold_document([WindowInput(WINDOW, ["Aluminum", "Brass"], window_id="0:99")])
    ...
    assert first["form"] == "Aluminum" and first["mention_id"] == intro.mention_id
    assert first["page"] == "https://acme.example/materials" and first["window"] == 0
    assert "sent_form" not in first
    # 2026-09-05: `location` is the fold's own (code-derived); this window has
    # no heading above its lines, so it is null here and nothing is located.
    assert first["location"] is None and dump["summary"]["mentions_located"] == 0
