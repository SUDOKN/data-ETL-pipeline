    ...
    for i in range(0, len(chunk_items), BATCH_SIZE):
        ...
        for chunk_bounds, extraction_bundle in batch:
            ...
            if left_only_phrases:
                raise ValueError(
                    f"Phrases {left_only_phrases} found in relationship results:{extraction_bundle.llm_phrase_relationship_req_id} but not in screening results:{extraction_bundle.llm_phrase_relationship_screening_req_ids} for {mfg_etld1}:{field_type} chunk {chunk_bounds}"
                )
            right_only_phrases = (
                llm_phrase_relationship_screening_results.keys()
                - llm_phrase_relationship_results.keys()
            ...
            if right_only_phrases:
                raise ValueError(
                    f"Phrases {right_only_phrases} found in screening results:{extraction_bundle.llm_phrase_relationship_screening_req_ids} but were never listed in relationship results:{extraction_bundle.llm_phrase_relationship_req_id} for {mfg_etld1}:{field_type} chunk {chunk_bounds}"
                )
