...
def get_file_name_from_subject_unique_id(subject_unique_id: str) -> str:
    ...
    extracted = tldextract.extract(
        subject_unique_id
    )  # because the subject_unique_id passed may not exactly be
    ...
    if reconstructed_etld1 != subject_unique_id:
        raise ValueError(
            f"subject_unique_id:{subject_unique_id} passed is inconsistent with reconstructed_etld1:{reconstructed_etld1}"
        )

    return f"{subject_unique_id}.txt"
