...
TEST_LLM_MODEL = GPT_4o_mini


@pytest.fixture(autouse=True)
def _default_token_counter(monkeypatch):
    ...
    monkeypatch.setattr(chunk_util.litellm, "token_counter", fake_token_counter)


def _set_fixed_token_counter(
    monkeypatch, full_text: str, total_tokens: int, line_tokens: int
):
    def fake_token_counter(model, text):
        if text == full_text:
            return total_tokens
...
def test_get_roughly_even_chunks_empty_text():
    result = get_roughly_even_chunks("", llm_model=TEST_LLM_MODEL)
    assert isinstance(result, dict)
    assert result == {}
...
def test_get_roughly_even_chunks_text_under_target(monkeypatch):
    ...
    result = get_roughly_even_chunks(
        text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=100,
        overlap_ratio=0.25,
    )
...
def test_get_roughly_even_chunks_calculates_divisions_correctly(monkeypatch):
    ...
    text = "Line1\nLine2\nLine3\nLine4\nLine5\nLine6"
    _set_fixed_token_counter(monkeypatch, text, total_tokens=6000, line_tokens=1000)
    ...
    result = get_roughly_even_chunks(
        text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=2500,
        overlap_ratio=0.25,
    )
...
def test_get_roughly_even_chunks_with_large_target(monkeypatch):
    ...
    result = get_roughly_even_chunks(
        text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=10000,
        overlap_ratio=0.25,
    )
...
def test_get_roughly_even_chunks_division_calculation(monkeypatch):
    ...
    _set_fixed_token_counter(monkeypatch, text, total_tokens=15000, line_tokens=100)
    ...
    result = get_roughly_even_chunks(
        text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=4000,
        overlap_ratio=0.1,
    )
...
def test_get_roughly_even_chunks_passes_correct_parameters(monkeypatch):
    ...
    def mock_get_chunks_respecting_boundaries(
        text, llm_model, soft_limit_tokens, overlap_ratio, max_chunks
    ):
    ...
    _set_fixed_token_counter(monkeypatch, text, total_tokens=8000, line_tokens=1000)
    monkeypatch.setattr(
        chunk_util,
    ...
    result = get_roughly_even_chunks(
        text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=target_tokens,
...
def test_get_roughly_even_chunks_realistic_scenario(monkeypatch):
    ...
    _set_fixed_token_counter(monkeypatch, long_text, total_tokens=200, line_tokens=10)
    result = get_roughly_even_chunks(
        long_text,
        llm_model=TEST_LLM_MODEL,
        max_tokens_allowed_per_chunk=100,
...
def test_get_roughly_even_chunks_integer_division_behavior(monkeypatch):
    ...
    target_tokens = 3333
    _set_fixed_token_counter(
        monkeypatch, "test text", total_tokens=10000, line_tokens=100
    )
    monkeypatch.setattr(
        chunk_util,
        "get_chunks_respecting_line_boundaries_sync",
...
def test_get_roughly_even_chunks_validates_chunk_count_and_sizes(monkeypatch):
    ...
    _set_fixed_token_counter(monkeypatch, text, total_tokens=1000, line_tokens=100)
    target_tokens = 300
    ...
    chunk_sizes = []
    for chunk_key, chunk_text in result.items():
        chunk_tokens = len(chunk_text.splitlines()) * 100
...
def test_get_roughly_even_chunks_specific_division_scenarios(monkeypatch):
    ...
    for case in test_cases:
        ...
        result = get_roughly_even_chunks(
            "test text",
            llm_model=TEST_LLM_MODEL,
            max_tokens_allowed_per_chunk=case["target_tokens"],
        )
