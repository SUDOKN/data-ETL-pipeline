...
from models.skos_concept import ConceptJSONEncoder
...
def get_industry_concept_nodes():
    try:
        return ontology_service.industry_capability_concept_nodes
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_certificate_concept_nodes():
    try:
        return ontology_service.certificate_capability_concept_nodes
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_process_capabilities():
    try:
        ...
        return JSONResponse(content=json.loads(json.dumps(concepts, cls=ConceptJSONEncoder)))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_industry_capabilities():
    try:
        concepts = ontology_service.industry_capabilities
        return JSONResponse(content=json.loads(json.dumps(concepts, cls=ConceptJSONEncoder)))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/ontology/certificate", response_class=JSONResponse)
def get_certificate_capabilities():
    try:
        concepts = ontology_service.certificate_capabilities
        return JSONResponse(content=json.loads(json.dumps(concepts, cls=ConceptJSONEncoder)))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
