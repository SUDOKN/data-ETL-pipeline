...
async def collect_keyword_extraction_ground_truth(
    ...
    manufacturer = await find_manufacturer_by_etld1(keyword_gt.subject_unique_id)
    if not manufacturer:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Manufacturer not found for etld1: {keyword_gt.subject_unique_id}. "
    ...
    if existing_keyword_gt:
        if not existing_keyword_gt.corrections:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Existing keyword ground truth for mfg_url: {keyword_gt.subject_unique_id}, "
