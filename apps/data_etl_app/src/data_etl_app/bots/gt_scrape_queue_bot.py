...
class NoScrapedFileFoundError(Exception):
    """Raised when no valid scraped file exists for a manufacturer on S3."""

    def __init__(self, mfg_etld: str):
        self.mfg_etld = mfg_etld
        super().__init__(
            f"No valid scraped file found for {mfg_etld}. Please register through the main app first."
        )
    ...
    if manufacturer:
        ...
        try:
            ...
            if item.email_errand:
                await item.email_errand.run_errand(
                    subject=subject,
                    html_content=f"<p>Error occurred: {str(e)} Please try again later.</p>",
                )
            await ScrapingError.insert_one(
                ScrapingError(
                    created_at=polled_at,
    ...
    if not existing_scraped_file:  # try to find any version on S3
        ...
        if latest_version_id:
            ...
            try:
                ...
                subject = f"GT: Error downloading existing scraped file for {mfg_etld}, version {latest_version_id}."
                if item.email_errand:
                    await item.email_errand.run_errand(
                        subject=subject,
                        html_content=f"<p>Error occurred: {str(e)} Please try again later.</p>",
...
async def async_main():
    ...
    from apps.data_etl_app.src.data_etl_app.utils.aws.queue.gt_extract_queue_util import (
        push_item_to_gt_extract_queue,
    )
    from apps.data_etl_app.src.data_etl_app.utils.aws.queue.gt_scrape_queue_util import (
        poll_item_from_gt_scrape_queue,
