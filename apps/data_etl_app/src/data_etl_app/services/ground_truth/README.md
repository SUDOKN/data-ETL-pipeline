...
- **Next action:** **P3 gate run IN PROGRESS (2026-08-18)** — instrument:
  [API_TEST_PLAN.md](API_TEST_PLAN.md) + the generated Postman collection,
  subject alecmfg.com (run `20260819T012442`, all 7 fields, no catalog
  drift). First finding fixed same day: sentinel declinations
  (`OovGroundingGT.llm_declined`, journal row below) — user re-runs B10 and
  continues. P3.7 still awaiting audit; user's own count/archive/drop of the
  two legacy Mongo collections still owed. Suite 1117 green. **No open
  forks.**
...
7. **Audit entries:** `agree | agree-but | disagree`. `agree-but` = correct but
   ...
   both are kept. **Extended twice 2026-08-19 (user, at the P3 gate), landing on
   TWO audit kinds:**
   - **`TextFieldAudit`** — prose surfaces (relationship text, rule audits).
     As above, plus optional free-form `note` on `agree_but`/`disagree`
     (forbidden on plain `agree`, non-blank when given) for rationale beside
     the addendum/replacement.
   - **`EntityFieldAudit`** — identifier surfaces (screening's
     `identified_entity`, grounding tags, in-vocab node tags, divergence
     hops). NO `corrected_text` at all: the asserted value lives on the
     derivation itself (`identified_entity` / `tag`), so there is no duplicate
     to keep coherent — the old `corrected_text == replacement` equality check
     is deleted, not extended. Verdicts are `agree | disagree` only
     (`agree_but` does not map to an identifier: missing groundings are
     asserted structurally, caveats on a kept identifier belong on the
     specific rule). `note` is REQUIRED non-blank on `disagree` (the only
     prose slot — why the original is wrong and the replacement right) and
     forbidden on `agree`.
   The intermediate design (one model, note optional everywhere) was built and
   superseded the same day when the user observed `corrected_text` was doing
   two jobs; freehand `disagree.corrected_text` had already been rejected for
   splitting the field's meaning across surfaces.
...
| 2026-08-18 | **P3 gate run STARTED (Postman) — first live finding found and fixed: sentinel declinations.** Test instrument: [API_TEST_PLAN.md](API_TEST_PLAN.md) (~100 cases, topological) + generated Postman collection (`LLM_Phrase_GT_API.postman_collection.json`), subject alecmfg.com run `20260819T012442` (all 7 fields; pinned catalog versions verified equal to deployed — no drift). Keyword template (B6) built clean; concept template (B10, conformity_attestations) failed: 400 `"tag 'None of the above' has no rules"`. Root cause: initial grounding's escape hatch is stored FAITHFULLY as a sentinel key with zero rules in `llm_phrase_initial_grounding` stats (13–119 per concept field this run) — a process record, not corruption — and P2.1's `inflate_tag_groundings` predates any sentinel fixture. Mid-descent declinations (`stop_reason="sentinel"` stored nodes) already inflated fine; only the tag-map path raised. Fix (user picked option 3 of skip/synthesize-node/model-field): core `OovGroundingGT.llm_declined=False` (old docs load; the declination joins the surviving record; sentinel never becomes a tag entry; browse stays thin per verdict 5a — the flag rides the detail trail + `?full=true`) + `split_sentinel_tag_rules` in catalog_template_inflation (sentinel keys → flag; sentinel-with-rules warns and drops, no catalog tree exists for a non-label; real zero-rule tags still raise) + `build_llm_phrase_gt_template` wraps `InflationError` → `TemplateAssemblyError` so the route 409s instead of the app-wide ValueError handler flattening it to a generic 400. SECOND finding recorded, not fixed: that global handler also makes P3.6's "route-bug ValueErrors → 500" wrong at runtime (they'd 400) — plan WATCH-5/M2. 8 new tests (split unit ×3, chunk ×2, model default ×1, app template ×2). B10 re-run PASSED. Follow-up (user probed visibility): `PhraseTruth.llm_declined` passthrough in gt_fold — the folded effective/truth plane collapsed "LLM declined" and "stage never answered the phrase" into identical empty `tags` (only the trail could tell them apart); the flag now flows to detail `effective`, truth GET, and the submission response's truth block with zero route/view changes, and a human tag asserted against a declination keeps it (contradiction stays on the record; +2 fold tests, +1 assert). Browse stays thin per verdict 5a. Suite 1119 green; pyright 0. Next: user continues the gate run (C10b verifies both flag surfaces). |
| 2026-08-19 | **Audit `note` field added mid-gate (user's call during surface-2 testing).** The gap the user hit: on an entity/tag replacement, `_check_replacement_coherence` pins `disagree.corrected_text` verbatim to the replacement, leaving no home for WHY — the fresh tree's rule explanations argue the NEW entity in, nothing argues the old one out. Freehand `corrected_text` on disagree was weighed and rejected: `effective_text` consumes it as the replacement on text surfaces (one field, two meanings by surface), and the equality check is the typo tripwire (echo-surfaces lesson). Built instead: optional `note` on `TextFieldAudit` — forbidden on plain `agree`, non-blank when given ([audits.py](../../../../../../packages/core/src/core/models/ground_truth/audits.py)); `EffectiveText.note` rides the latest-wins fold ([gt_fold.py](../../../../../../packages/core/src/core/services/ground_truth/gt_fold.py)); browse rows gain `relationship_note` ([llm_phrase_gt_view_service.py](llm_phrase_gt_view_service.py)). Compact truth models (ScreeningTruth/TagTruth/InVocabNodeTruth) deliberately stay note-free — the detail trail serves raw audits, so entity/tag/rule notes are already visible there; revisit only if annotation practice wants them folded. Settled semantics #7 amended; `llm_phrase_ground_truth.schema.json` regenerated (note at every TextFieldAudit embedding). Additive — in-flight Postman bodies stay valid. 10 new test cases; suite 1129 green; pyright 0 on all touched files. Test-plan cases D11/D12 (note requiredness 422s) and E12 (note surfaces in browse) added. |
| 2026-08-19 | **Audit model SPLIT into two kinds (user's design, same session as the note field): `EntityFieldAudit` for identifier surfaces.** The user's observation mid-testing: `corrected_text` was doing two jobs — free replacement prose on text surfaces, a machine-pinned verbatim duplicate on entity/tag surfaces — "using corrected text for both cases sounds wrong". Verdict: split. [audits.py](../../../../../../packages/core/src/core/models/ground_truth/audits.py) gains `EntityFieldAudit {type, note, author_email, at, source}`: verdicts `agree|disagree` ONLY (`agree_but` doesn't map to an identifier — missing groundings are structural assertions, kept-identifier caveats belong on rules), `note` REQUIRED non-blank on disagree (the only prose slot), forbidden on agree, and NO `corrected_text` — the asserted value lives on the derivation (`identified_entity`/`tag`), so the P2.3 equality check (`corrected_text == replacement`) is DELETED rather than extended; nothing is left to drift, and the G8/G9 ontology-classification echo still guards membership typos. `HumanScreeningDerivation.audits` + `GroundingDerivation.audits` re-typed (screening entity, oov/initial tags, in-vocab node keeps, divergence hops all ride the latter). `_check_replacement_coherence(audits, what)` now checks only latest-is-disagree; `_inner_audits`/`_check_single_author` widened to the union. Fold and rollup untouched (verified: they never read identifier corrected_text). Test-plan fallout: F6f (corrected_text ≠ entity → 400) is now IMPOSSIBLE BY CONSTRUCTION and deleted; identifier-audit 422 cases added. Schema regenerated (derivation audit items: `{type, note, author_email, at, source}`). Suite 1138 green; pyright 0. Postman bodies for F/G/H/I simplify — the user's in-flight collection needs its entity-audit objects reshaped (corrected_text → note). |
