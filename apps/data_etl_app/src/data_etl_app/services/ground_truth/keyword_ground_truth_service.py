...
async def get_keyword_ground_truth(
    ...
    ), f"Keyword type '{keyword_type}' not found in manufacturer '{manufacturer.subject_unique_id}'."
    return await KeywordGroundTruth.find_one(
        KeywordGroundTruth.subject_unique_id == manufacturer.subject_unique_id,
        ...
        KeywordGroundTruth.metadata.search_prompt_version_id
        == keyword_extraction_results.metadata.search_prompt_version_id,
        # ---------------------------------------------------- #
...
async def add_new_correction_to_keyword_ground_truth(
    ...
    new_correction: HumanKeywordCorrection,
    timestamp: datetime,
) -> KeywordGroundTruth:

    existing_keyword_gt.updated_at = timestamp
    if (
        existing_keyword_gt.corrections[-1].human_correction.author_email
        == new_correction.author_email
    ):
        existing_keyword_gt.corrections.pop()  # the new correction will replace the last one because it is from the same author

    await _validate_keyword_ground_truth_correction(
        manufacturer, existing_keyword_gt, new_correction
    )

    existing_keyword_gt.corrections.append(
        KeywordCorrectionLog(
...
async def save_new_keyword_ground_truth(
    ...
    await _validate_keyword_ground_truth_correction(
        manufacturer, new_keyword_gt, new_correction
    )

    new_keyword_gt.corrections.append(
        KeywordCorrectionLog(
...
async def _validate_keyword_ground_truth_correction(
    manufacturer: AbstractExtractionSubject,
    ...
    if not keyword_extraction_results:
        raise ValueError(
            ...
            f" in manufacturer '{keyword_gt.subject_unique_id}'."
        )
    ...
    if keyword_gt.extraction_stats != chunk_search_stats:
        raise ValueError(
    ...
    if manufacturer.scraped_text_file_version_id != version_id:
        raise ValueError(
            f"Scraped text version ID mismatch for mfg_url: {keyword_gt.subject_unique_id}. Expected: {manufacturer.scraped_text_file_version_id}, got: {version_id}. (manufacturer.scraped_text_file_version_id != version_id)"
        )
...
def _validate_new_human_correction(
    ...
    for kw in new_correction.llm_search.add:
        if not re.search(word_regex(kw), chunk_keyword_gt.chunk_text):
            raise ValueError(
                f"Keyword '{kw}' to be added is not present in the chunk text."
            )
        if kw in chunk_keyword_gt.extraction_stats.llm_search:
            raise ValueError(
                f"Keyword '{kw}' to be added is already present in the extracted llm_search results."
            )

    for kw in new_correction.llm_search.remove:
        if kw not in chunk_keyword_gt.extraction_stats.llm_search:
            raise ValueError(
                f"Keyword '{kw}' to be removed is not present in the extracted llm_search results."
            )
