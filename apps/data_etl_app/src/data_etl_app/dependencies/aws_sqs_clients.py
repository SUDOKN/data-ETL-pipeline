...
class ScraperAWSClients:
    """Manages AWS clients at the scraper application level (scrape + extract)."""
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            if self._initialized:
                return
            logger.info("Initializing scraper AWS clients...")
            ...
            logger.info("Scraper AWS clients initialized")
    ...
    async def cleanup(self) -> None:
        if not self._initialized:
            return
        async with self._lock:
            logger.info("Cleaning up scraper AWS clients...")
            ...
            self._initialized = False
            logger.info("Scraper AWS clients cleaned up")


# Keep the instance private; expose only functions.
_scraper_aws_clients = ScraperAWSClients()


async def initialize_scraper_aws_clients() -> None:
    await _scraper_aws_clients.initialize()


async def cleanup_scraper_aws_clients() -> None:
    await _scraper_aws_clients.cleanup()


def get_scraped_bucket_s3_client() -> S3Client:
    if _scraper_aws_clients.scraped_bucket_s3_client is None:
        raise RuntimeError(
            "S3 client not initialized. Call initialize_scraper_aws_clients() at app startup."
        )
    return _scraper_aws_clients.scraped_bucket_s3_client


def get_scrape_queue_client() -> SQSClient:
    if _scraper_aws_clients.scrape_queue_client is None:
        raise RuntimeError(
            "SQS scrape client not initialized. Call initialize_scraper_aws_clients() at app startup."
        )
    return _scraper_aws_clients.scrape_queue_client


def get_extract_queue_client() -> SQSClient:
    if _scraper_aws_clients.extract_queue_client is None:
        raise RuntimeError(
            "SQS extract client not initialized. Call initialize_scraper_aws_clients() at app startup."
        )
    return _scraper_aws_clients.extract_queue_client
