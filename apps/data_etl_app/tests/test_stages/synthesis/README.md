...
into `|ud=`, so synthesis regenerated by itself — delete block commented, search replayed). Mechanical: 228
first-pass requests (180 at radius 0; +27%), 5.24M input + 318k output tokens ≈ $13 (4.02M + 280k ≈ $10),
paragraphs +10–30% per field (mathewsco equipments 359 → 530 chars, tanfel industries 474 → 593), 0 invariant
failures, 0 own-designation drops, 0 focal-form-absent, twins unchanged, sibling-mention flat. Retried records
5 → 62: not the designation trigger — one 42-record mathewsco products request answered ONE record and closed
the array (137 output tokens; the one-and-done delivery mode, first seen on the label wire, now once in 180
requests of the plain wire) and the retry pass recovered all 41; the trigger itself fired on a handful.
Judged: the same 1,494 keys as every earlier draw (`judged_population.py … --singletons 4000`), 10 Sonnet jobs
(1,494/1,494, 0 malformed, 0 duplicates), the run's `JUDGE_PROMPT.md` carries a radius note (a neighbouring
line's statement about the focal entity is snippet evidence; narrating the neighbour's own item is a swap),
1 Opus packet (98 items: 23 majors, 75 passes; 14 records changed — J1 pass→major 8 completing the laundering
bundle on the line-card records, J6 major→pass 2 by the field-membership gate, J2 pass→minor 2, J1 major→minor
1), `apply_corrections`, `finalize`. The readout pairs BY KEY (`paired_readout.py --by-key --by-subject`):
every pair's evidence changed, so the flips carry the lever AND the floor together.

**Paired against the three radius-0 draws of the same subjects (verified):**

| readout | vs draw 1 (191548) | vs draw 2 (225723) | vs draw 3 (041535) |
|---|---|---|---|
| pairs by key | 1,492 | 1,473 | 1,466 |
| records with any fail | 3.2% → **2.0%** (47 → 30) | 5.8% → 2.0% (85 → 30) | 5.3% → 2.0% (77 → 30) |
| records with a major | 2.1% → **1.4%** (32 → 21) | 3.9% → 1.4% (58 → 21) | 2.9% → 1.4% (43 → 21) |
| mathewsco (353): any-fail / major | 22 → 18 / 20 → 14; flips 11.3% / 9.6% | 43 → 18 / 34 → 14; flips 14.4% / 10.8% | 35 → 18 / 33 → 14; flips 11.6% / 9.9% |
| tanfel: any-fail / major | 25 → **12** / 12 → 7; flips 2.5% (+8 −21) / 1.1% | 42 → 12 / 24 → 7; flips 3.0% (+2 −32) / 1.5% | 42 → 12 / 10 → 7; flips 3.8% (+6 −36) / 1.0% |
| ALL flips any-fail / major | 4.6% (+26 −43) / 3.2% | 5.8% (+15 −70) / 3.7% | 5.7% (+18 −65) / 3.1% |

Per-request clustering (`request_mode_readout.py`): **0% of the 21 majors sit in majority-failing requests,
0 whole-request fails** (radius-0 draws: 22% / 51% / 35%, 0 / 1 / 3). The 8 records that failed in EVERY
radius-0 draw (all tanfel process_caps, the bare-noun capabilities index): **4 now pass** (Membrane Switch,
the two Fused Filament Fabrication records, the thread-plug-gauge cross-link) and 4 still fail (the three
`Metal: <process>` index lines and the metrology heading — TF-P7 4 of 12). Probe TF-P8 (C25, the testimonial
whose attribution sat one line past the snippet): **0 fail / 4** — fixed; TF-P3's containment class (the
spring list's introducing sentence) dissolved into snippet evidence (3 records pass). Records that passed in
all three radius-0 draws and fail now: 9 — four are the sales-rep coin landing on "launder" for the
cold-heading line-card entries (MW-P1, products), the rest singletons (a company name invented from a bare
domain `johnsoncentrifugal.com`; 'Energy' J2 minor; 'die forging' fragment J1 minor; 'Ceramic coating' J2
minor; 'Casting' J1 minor). Remaining major classes: MW-P4 (6, the subject-elided kpl05 fragment — unchanged;
the SAME sentence reads as a third party's in conformity and as Mathews' own in products/material_caps, and
tracks the request, not the sentence), the line-card laundering (8, the coin), tanfel nickel alloys (4: the
asserting line "Our inventory of metals and alloys includes:" sits SEVEN bullets above — radius 1 does not
reach it), TF-P7 (3).

**Radius-1 evidence artifacts (new, C28):** the PREVIOUS list item's trailing standard mark rides into the
next record's snippet ("ISO 9001\n\nkpl05 Rubber Fabrication…") — the mechanical `designations_dropped`
nominator fires on it on every Kansas-line-card record (verified false positives; the model itself never
credited it); on a testimonials page where the "- Name, Title" citation FOLLOWS its quote, the wider snippet
pulls the previous quote's citation in as if it introduced this one, and one record credits the wrong customer
(tanfel industries gk7hao9w — judged pass because the paragraph is faithful to its evidence; the error is
upstream). Probe TF-P9.

**Reading.** (1) The first draw at radius 1 is the lowest-fail draw measured on these subjects: 2.0% any-fail
/ 1.4% majors against 3.2 / 5.8 / 5.3% and 2.1 / 3.9 / 2.9%. (2) On the maker site the move is beyond the
floor and one-directional (25/42/42 → 12 any-fail; newly passing 21–36 against newly failing 2–8; floor 3.5%);
on the representative the flips (11.3–14.4%) sit inside the three-way spread (8.2–21.5%) — still the coin.
(3) Fails stopped clustering by request in this draw (0% vs 22–51%); one draw cannot say whether wider
evidence makes the per-request mode rarer — draw 2 (delete block LIVE, same two subjects) against this draw
is the radius-1 floor and decides. (4) Cost +30% input tokens, +27% requests, paragraphs +10–30%. (5) The
evidence-side classes moved as predicted where the answer was ONE unit away (TF-P8, TF-P3, half of the
unanimous fails) and not where it was further (nickel alloys, `Metal:` index) — the lever is the distance.
Decision after draw 2 (design doc §44): adopt radius 1 if the radius-1 floor is not wider than radius 0's and
draw 2 stays under the radius-0 draws.
