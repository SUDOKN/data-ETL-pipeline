...
def needs_processing(
    mfg: Manufacturer, deferred_mfg: Optional[DeferredManufacturer]
) -> bool:
    """
    Check if manufacturer/deferred_manufacturer pair needs processing.

    Returns True if at least one field is missing in BOTH manufacturer and deferred_manufacturer.
    This matches the logic in upsert_deferred_manufacturer service.
    ...
    - material_caps
    """
    # If no deferred manufacturer exists, we need to process
    if not deferred_mfg:
        return True

    # Check each field - need processing if BOTH are missing/None
    checks = [
        (not mfg.is_manufacturer and not deferred_mfg.is_manufacturer),
        (
            not mfg.is_contract_manufacturer
            and not deferred_mfg.is_contract_manufacturer
        ),
        (not mfg.is_product_manufacturer and not deferred_mfg.is_product_manufacturer),
        (not mfg.addresses and not deferred_mfg.addresses),
        (not mfg.business_desc and not deferred_mfg.business_desc),
        (not mfg.products and not deferred_mfg.products),
        (not mfg.certificates and not deferred_mfg.certificates),
        (not mfg.industries and not deferred_mfg.industries),
        (not mfg.process_caps and not deferred_mfg.process_caps),
        (not mfg.material_caps and not deferred_mfg.material_caps),
    ]

    # Return True if ANY field needs processing
    return any(checks)
...
async def process_manufacturers_with_dynamic_concurrency(
    ...
    async def fetch_next_batch():
        ...
        nonlocal processed, failure_count, failed_etld1s
        nonlocal s3_version_error_count, duplicate_key_error_count, other_error_count
        nonlocal s3_version_error_etld1s, duplicate_key_error_etld1s, other_error_etld1s
        ...
        try:
            ...
            # Create manufacturer lookup map for efficient access
            mfg_map = {
                (m.etld1, m.scraped_text_file_version_id): m
            ...
            # Filter to manufacturers that actually need processing (include those with no deferred doc)
            deferred_map = {
                (dm.mfg_etld1, dm.scraped_text_file_version_id): dm
                for dm in existing_deferred_mfgs
            }

            work_items = []  # list of tuples: (mfg, existing_deferred_or_none)
            skipped_count = 0
            new_count = 0
            existing_count = 0

            for mfg in batch_manufacturers:
                key = (mfg.etld1, mfg.scraped_text_file_version_id)
                existing_deferred = deferred_map.get(key)

                if needs_processing(mfg, existing_deferred):
                    work_items.append((mfg, existing_deferred))
                    ...
                    skipped_count += 1
                    logger.debug(f"Skipping {mfg.etld1} - already complete")

            logger.info(
                f"Filtered to {len(work_items)} manufacturers needing processing "
                f"(skipped {skipped_count} already complete; new={new_count}, existing={existing_count})"
            )

            # Download S3 files in parallel for those that need processing
            logger.info(
                f"Downloading S3 files for {len(work_items)} manufacturers in parallel..."
            )
            s3_download_tasks = [
                ScrapedTextFile.download_from_s3_and_create(
                    mfg.etld1, mfg.scraped_text_file_version_id
                )
                for (mfg, _) in work_items
            ]
            ...
            s3_success_count = 0
            s3_failure_count = 0
            for (mfg, existing_deferred), result in zip(work_items, s3_results):
                ...
                if isinstance(result, Exception):
                    ...
                    if exception or not scraped_file:
                        ...
                        # S3 download succeeded - add to queue with the existing deferred manufacturer
                        s3_success_count += 1
                        manufacturers_queue.append(
                            (mfg, scraped_file, existing_deferred)
                        )
                else:
    ...
    while total_fetched < total_count or manufacturers_queue:
        ...
        if not manufacturers_queue:
            if total_fetched < total_count:
                # If nothing queued and not currently fetching, kick off next fetch
                if not is_fetching and not prefetch_triggered:
                    logger.info("Queue empty, triggering fetch...")
                    asyncio.create_task(fetch_next_batch())
